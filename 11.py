...
current_argv=0
print(argc , "adsasd")
while ((argc==1) and current_argv>0 and (sys.argv[1][0] == '-')):#maybe change after
  argc-=1
  s=sys.argv[1][1]
  if s=='m':
    magic=1
    current_argv+=1
    magic_layer+=sys.argv[1][1]
    argc-=1
  elif s=='c':
    cif=1
    current_argv+=1
    output_file +="logo.cif"
    cif_layer +=sys.argv[1][1]
    argc-=1
  elif s=='t':
    argc-=1
    current_argv+=1
    magic_tech+=sys.argv[1][1]
  elif s=='v':
    view_array=int(sys.argv[1][1])
    argc-=1
  elif s=='A':
    thresh_after=int(sys.argv[1][1])
    smooth_after=1
    argc-=1
  elif s=='B':
    thresh_before=int(sys.argv[1][1])
    smooth_before=1
    argc-=1
  elif s=='h':
    error_mess()
    ...
    min_width=int(sys.argv[1][1])
    argc-=1
  elif s=='s':
    scale_factor=float(sys.argv[1][1])
    argc-=1
  else:
    break
print(sys.argv[1])
